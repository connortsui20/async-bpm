use async_bpm::{page::PAGE_SIZE, replacer::Fifo, BufferPoolManager};
...
fn throughput<const ZIPF: bool>() {
    let bpm = BufferPoolManager::<Fifo>::new(FRAMES, STORAGE_PAGES);
    let bpm = Arc::new(bpm);
        ...
        for thread in 0..THREADS {
            ...
            s.spawn(move || {
                ...
                BufferPoolManager::<Fifo>::start_thread(async move {
                    ...
                    for task in 0..TASKS {
                        let bpm = bpm.clone();
                        let handle = spawn_bench_task::<ZIPF>(bpm, barrier.clone(), thread, task);
                        set.spawn(handle);
                    }
...
fn spawn_bench_task<const ZIPF: bool>(
    bpm: Arc<BufferPoolManager<Fifo>>,
    barrier: Arc<Barrier>,
    _thread: usize,
    _task: usize,
) -> JoinHandle<()> {
    ...
    BufferPoolManager::<Fifo>::spawn_local(async move {
        ...
        for _ in 0..ITERATIONS {
            ...
            let ph = bpm.clone().get_page(&id).await.unwrap();
